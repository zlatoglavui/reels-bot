...
новость → сценарий → аудио → видео → отправить в Telegram личку
"""
import asyncio
import os
import shutil
...
from src.video.composer import compose_video, download_music
...
class ReelsPipeline:
    ...
    async def startup(self):
        ...
        # Сбрасываем ошибочные reels
        recovered = await self.db.reset_error_reels()
        if recovered > 0:
        ...
        if bg_dir.exists():
            ...
            logger.info("Кэш фонов очищен")

        # Скачиваем музыку для фона
        await download_music()

        # Запускаем Telegram команды
        await self.sender.start_polling()

        logger.info("Reels Pipeline запущен ✓")

    async def shutdown(self):
        await self.sender.stop_polling()
        await self.db.close()
    ...
    async def process_one(self, article: dict) -> bool:
        ...
        # Шаг 5: Отправка в Telegram личку
        caption = (
            f"🎬 {script['hook']}\n\n"
            f"📰 {title[:100]}\n\n"
            f"#reels #финансы"
        ...
        # Автоочистка старых аудио
        self.sender.cleanup_audio()
    ...
    async def run_once(self):
        ...
        if not articles:
            logger.info("Нет опубликованных новостей")
            return

        new_articles = [a for a in articles if not await self.db.reel_exists(a["id"])]
        if not new_articles:
        ...
        logger.info(
            ...
            f"сегодня={today_count + stats['success']}/{VIDEOS_PER_DAY}"
        )
